...
void mcpwm_example_gpio_initialize(int PWM_Pin)
{
    ...
    mcpwm_gpio_init(MCPWM_UNIT_0, MCPWM0A, PWM_Pin);    //Set GPIO 18 as PWM0A, to which servo is connected
}
...
void mcpwm_example_servo_control(void *arg)
{
    ...
    while (1) {
        ...
        mcpwm_set_duty_in_us(MCPWM_UNIT_0, MCPWM_TIMER_0, MCPWM_OPR_A, 1390);
        vTaskDelay(100 / portTICK_PERIOD_MS);
        while(gpio_get_level(BUTTON_GPIO)==0);
        while(gpio_get_level(BUTTON_GPIO)!=0);

        mcpwm_set_duty_in_us(MCPWM_UNIT_0, MCPWM_TIMER_0, MCPWM_OPR_A, 2380);
        vTaskDelay(100 / portTICK_PERIOD_MS);
        while(gpio_get_level(BUTTON_GPIO)==0);
        while(gpio_get_level(BUTTON_GPIO)!=0);
